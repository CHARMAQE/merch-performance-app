...
SELECT COUNT(*) FROM stores s JOIN visits v ON s.store_id = v.store_id WHERE v.visit_date = '2026-04-01' AND s.store_city = 'Casablanca';    


SELECT COUNT(DISTINCT store_code) AS city_casablanca_stores
FROM stores
WHERE UPPER(TRIM(store_city)) = 'CASABLANCA';


SELECT COUNT(DISTINCT store_code) AS region_casablanca_stores
FROM stores
WHERE UPPER(TRIM(store_region)) = 'CASABLANCA';

SELECT store_city, COUNT(DISTINCT store_code) AS stores
FROM stores
WHERE UPPER(TRIM(store_region)) = 'CASABLANCA'
GROUP BY store_city
